...
def get_parser():
    ...
    parser.add_argument(
        "--pose_file", type=str, default="./eval_constant/sequences/house-dance.npy"
    )
    parser.add_argument("--model_path", type=str, default="./pre_trained")
    parser.add_argument(
        "--obj_path", type=str, default="./eval_constant/meshes/artist-2.obj"
    )
...
def load_model(
    device, model_args, topo_loader, save_path_base, envelope_only, epoch_num=-1
):
    """
    ...
    geo, att, gen = create_envelope_model(
        device, model_args, topo_loader, is_train=False, parents=parent_smpl
    )
    ...
    if not envelope_only:
        ...
        residual_model = BlendShapesGenerate(
            geo2, att, gen2, coff, args=model_args, fk=fk
        )
...
def write_back(
    prefix, skeleton, skinning_weight, verts, faces, original_path, rot, basis, coff
):
...
def main():
    ...
    # Read from eval_constant/sequences/*.npy
    if args.pose_file.endswith("npy"):
        test_pose, _ = load_test_anim(args.pose_file, device)
    ...
    env_model, res_model = load_model(
        device, model_args, topo_loader, args.model_path, args.envelope_only
    )
